...
async function addRide(req, res) {
  ...
  switch(model.myDatabase[id].status) {
    case 0:
    model.myDatabase[id].loyaltyPoints = model.myDatabase[id].loyaltyPoints + parseInt(req.body.price);
    break;
    case 1:
    model.myDatabase[id].loyaltyPoints = model.myDatabase[id].loyaltyPoints + ( 3 * parseInt(req.body.price) );
    break;
    case 2:
    model.myDatabase[id].loyaltyPoints = model.myDatabase[id].loyaltyPoints + ( 5 * parseInt(req.body.price) );
    break;
    case 3:
    model.myDatabase[id].loyaltyPoints = model.myDatabase[id].loyaltyPoints + ( 10 * parseInt(req.body.price) );
    break;
  }
